...
def test_alphabetical_pick_when_no_child_blocks(fork_env):
    """
    When no child blocks are available, side/main chains can't be
    told appart. Watcher should pick the first header in alphabetical
    order.
    This is for consistency across watcher instances because
    nodes can report blocks at same height in different order, so relying
    on index only can yield different blocks depending on the node.
    """
    db_conn, cfg_path = fork_env
    ...
    # Step before first run, now 672220_fork and 672220 are visible
    r = requests.get(f"http://localhost:9053/step")
    assert r.status_code == 200

    # First run. Should pick block 672220_fork
    cp = run_watcher(cfg_path)
    assert cp.returncode == 0
