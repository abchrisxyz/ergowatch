use crate::parsing::BlockData;
use log::info;
use postgres::Client;
use postgres::Transaction;

use super::heights::Cache as HeightsCache;
use super::utils::bootstrap_change_summary;
use super::utils::refresh_change_summary;
use super::Cache;

const SUMMARY_COLUMNS: &[&'static str] = &["daily_1d", "daily_7d", "daily_28d"];

pub(super) fn include(tx: &mut Transaction, block: &BlockData, cache: &Cache) {
    tx.execute(
        sql::INSERT_SNAPSHOT,
        &[
            &block.height,
            &cache.height_1d_ago,
            &cache.height_7d_ago,
            &cache.height_28d_ago,
        ],
    )
    .unwrap();
}

pub(super) fn rollback(tx: &mut Transaction, block: &BlockData) {
    tx.execute(sql::DELETE_SNAPSHOT, &[&block.height]).unwrap();
}

pub(super) fn refresh_summary(tx: &mut Transaction, hc: &HeightsCache) {
    refresh_change_summary(tx, hc, "mtr.volume", &SUMMARY_COLUMNS);
}

pub fn bootstrap(client: &mut Client, work_mem_kb: u32) -> anyhow::Result<()> {
    if !is_bootstrapped(client) {
        do_bootstrap(client, work_mem_kb)?;
    }
    if !constraints_are_set(client) {
        set_constraints(client);
    }
    Ok(())
}

fn do_bootstrap(client: &mut Client, work_mem_kb: u32) -> anyhow::Result<()> {
    info!("Bootstrapping metrics - volume");

    let mut tx = client.transaction()?;

    tx.execute(&format!("set local work_mem = {};", work_mem_kb), &[])?;

    // Prep a work table with both height and timestamps
    tx.execute(
        "
        create table tmp_volume as
           select h.height
               , h.timestamp
               , s.volume
           from blk.stats s
           join core.headers h on h.height = s.height
           order by 1;
        ",
        &[],
    )?;

    tx.execute("create index on tmp_volume(timestamp);", &[])?;
    tx.execute("alter table mtr.volume set unlogged;", &[])?;
    tx.execute(
        "
        insert into mtr.volume (height, daily_1d, daily_7d, daily_28d)
        select height
            , (
                select sum(volume)::bigint
                from tmp_volume
                where timestamp > t.timestamp - 86400000::bigint
                    and timestamp <= t.timestamp
            ) as daily_1d
            , (
                select (sum(volume) / 7)::bigint
                from tmp_volume
                where timestamp > t.timestamp - (86400000::bigint * 7)
                    and timestamp <= t.timestamp
            ) as daily_7d
            , (
                select (sum(volume) / 28)::bigint
                from tmp_volume
                where timestamp > t.timestamp - (86400000::bigint * 28)
                    and timestamp <= t.timestamp
            ) as daily_28d
        from tmp_volume t
        order by 1;
        ",
        &[],
    )?;

    // Summary table
    bootstrap_change_summary(&mut tx, "mtr.volume", &SUMMARY_COLUMNS);

    tx.execute("alter table mtr.volume set logged;", &[])?;
    tx.execute("drop table tmp_volume;", &[])?;
    tx.commit()?;
    Ok(())
}

fn is_bootstrapped(client: &mut Client) -> bool {
    client
        .query_one("select exists(select * from mtr.volume limit 1);", &[])
        .unwrap()
        .get(0)
}

fn constraints_are_set(client: &mut Client) -> bool {
    client
        .query_one("select volume_constraints_set from mtr._log;", &[])
        .unwrap()
        .get(0)
}

fn set_constraints(client: &mut Client) {
    let statements = vec![
        "alter table mtr.volume add primary key(height);",
        "alter table mtr.volume alter column height set not null;",
        "alter table mtr.volume alter column daily_1d set not null;",
        "alter table mtr.volume alter column daily_7d set not null;",
        "alter table mtr.volume alter column daily_28d set not null;",
        "update mtr._log set volume_constraints_set = TRUE;",
    ];
    let mut tx = client.transaction().unwrap();
    for statement in statements {
        tx.execute(statement, &[]).unwrap();
    }
    tx.commit().unwrap();
}

mod sql {
    pub(super) const INSERT_SNAPSHOT: &str = "
        insert into mtr.volume (height, daily_1d, daily_7d, daily_28d)
        select $1 as height
            ,  sum(volume) filter (where height >= $2) as daily_1d
            , (sum(volume) filter (where height >= $3) / 7)::bigint as daily_7d
            , (sum(volume) filter (where height >= $4) / 28)::bigint as daily_28d
        from blk.stats;";

    pub(super) const DELETE_SNAPSHOT: &str = "delete from mtr.volume where height= $1;";
}
