...
use super::votes;
use crate::node;
use crate::node::models::ADProofs;
use crate::node::models::Extension;
use crate::node::models::POWSolutions;
...
impl From<node::models::Block> for RenderedBlock {
    fn from(block: node::models::Block) -> Self {
        Self {
            header: block.header.into(),
            transactions_size: block.block_transactions.size as i32,
            transactions: block
                .block_transactions
                .transactions
                .into_iter()
                .enumerate()
                .map(|(i, tx)| Transaction::from((tx, i)))
                .collect(),
            extension: block.extension,
            ad_proofs: block.ad_proofs,
        }
    }
}
...
impl From<node::models::Header> for Header {
    fn from(header: node::models::Header) -> Self {
        Self {
            ...
            votes: votes::from_str(&header.votes),
...
impl From<(node::models::Transaction, usize)> for Transaction {
    fn from((tx, index): (node::models::Transaction, usize)) -> Self {
        debug!("Processing transaction {}", &tx.id);
        Transaction {
            id: tx.id,
            index: index as i32,
            outputs: tx.outputs.into_iter().map(|op| Output::from(op)).collect(),
            input_box_ids: tx.inputs.into_iter().map(|i| i.box_id).collect(),
            data_input_box_ids: tx.data_inputs.into_iter().map(|d| d.box_id).collect(),
        }
    }
}
...
impl From<node::models::Output> for Output {
    fn from(output: node::models::Output) -> Self {
        debug!("Processing output {}", &output.box_id);
        let box_size = match calc_box_size(&output) {
            Some(s) => s,
            None => 0,
        };
        Output {
...
mod tests {
    ...
    #[test]
    fn header_info() -> () {
        let block = RenderedBlock::from(block_600k());
        let node_block = block_600k();
        assert_eq!(block.header.extension_id, node_block.header.extension_id);
        assert_eq!(
            block.header.difficulty.to_string(),
            node_block.header.difficulty
        );
        assert_eq!(block.header.height, node_block.header.height as i32);
        assert_eq!(block.header.id, node_block.header.id);
        assert_eq!(block.header.parent_id, node_block.header.parent_id);
        assert_eq!(block.header.timestamp, node_block.header.timestamp as i64);
        assert_eq!(
    ...
    fn transaction_from_node_tx() {
        let index = 1usize;
        let node_tx = block_600k().take_tx(index);
        let tx = Transaction::from((node_tx, index));
    ...
    fn output_from_node_output() {
        let node_output = block_600k().take_tx(1).take_output(0);
        let output = Output::from(node_output);
        let node_output = block_600k().take_tx(1).take_output(0);
    ...
    #[test]
    fn output_size() {
        // No assets, no additional registers
        let node_output = block_600k().take_tx(0).take_output(0);
        let output = Output::from(node_output);
...
//     /// Returns a BlockData with multiple outputs for a same
//     pub fn block_with_repeated_addresses<'a>() -> BlockData<'a> {
//         let tx_1 = Transaction {
//             id: "4ac89169a2f83adb895b3d76735dbcfc63ad7940bddc2492d9ee4201299bf927",
//             index: 0,
//             outputs: vec![Output {
