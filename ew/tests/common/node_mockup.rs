...
impl TestNode {
    ...
    async fn serve(&mut self, block_ids: &[&str]) {
        ...
        let app = Router::new()
            ...
            .route("/utxo/genesis", get(genesis_boxes))
...
async fn chain_slice(
    ...
    // While real node supports negative indices, we don't
    // and ew is never expected to request them.
    assert!(params.from_height >= 0);
    ...
    let start_h = std::cmp::max(0, start_h);
