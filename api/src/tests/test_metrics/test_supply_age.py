...
metrics.supply_age.TimeWindowLimits = limits
...
# overall, p2pks, cexs, contracts, miners
LAST_VAL = [600001, 600002, 600003, 600004, 600005]
NCOLS = len(LAST_VAL)
...
def client():
    ...
    sql = f"""
        ...
        -- values = height * 10 + 1
        insert into mtr.supply_age_days (height, overall, p2pks, cexs, contracts, miners) values 
        ...
        insert into mtr.supply_age_days_summary(
...
def base_url(r: str):
    return f"/metrics/supply/age?r={r}"
...
class TestSeriesApi:
    ...
    def test_from_to(self, client, r, dt, tss, vals):
        ...
        assert response.json() == {
            "timestamps": tss[1:4],
            "overall": [row[0] for row in vals[1:4]],
            "p2pks": [row[1] for row in vals[1:4]],
            "cexs": [row[2] for row in vals[1:4]],
    ...
    def test_from_spot(self, client, r, dt, tss, vals):
        ...
        assert response.json() == {
            "timestamps": tss[1:5],
            "overall": [row[0] for row in vals[1:5]],
            "p2pks": [row[1] for row in vals[1:5]],
            "cexs": [row[2] for row in vals[1:5]],
    ...
    def test_from_just_after_spot(self, client, r, dt, tss, vals):
        ...
        assert response.json() == {
            "timestamps": tss[4:6],
            "overall": [row[0] for row in vals[4:6]],
            "p2pks": [row[1] for row in vals[4:6]],
            "cexs": [row[2] for row in vals[4:6]],
    ...
    def test_default_ergusd(self, client, r, dt, tss, vals):
        url = base_url(r)
        url += "&ergusd=1"
        response = client.get(url)
        assert response.status_code == 200
    ...
    def test_from_to_ergusd(self, client, r, dt, tss, vals):
        url = base_url(r)
...
def test_summary(client):
    url = f"/metrics/summary/supply/age"
    response = client.get(url)
    assert response.status_code == 200
    assert response.json() == [
        {
