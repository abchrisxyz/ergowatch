...
def base_url(r: str):
    return f"/metrics/exchanges/supply?r={r}"
...
class TestSeriesApi:
    ...
    def test_from_to(self, client, r, dt, tss, vals):
        ...
        assert response.json() == {
            "timestamps": tss[1:4],
            "total": [row[0] for row in vals[1:4]],
            "deposit": [row[1] for row in vals[1:4]],
        }
    ...
    def test_from_spot(self, client, r, dt, tss, vals):
        ...
        assert response.json() == {
            "timestamps": tss[1:5],
            "total": [row[0] for row in vals[1:5]],
            "deposit": [row[1] for row in vals[1:5]],
        }
    ...
    def test_from_just_after_spot(self, client, r, dt, tss, vals):
        ...
        assert response.json() == {
            "timestamps": tss[4:6],
            "total": [row[0] for row in vals[4:6]],
            "deposit": [row[1] for row in vals[4:6]],
        }
...
def test_default_r_is_block(client):
    url = f"/metrics/exchanges/supply"
    response = client.get(url)
    assert response.status_code == 200
    assert response.json() == {
        "timestamps": [LAST_TS],
...
def test_summary(client):
    url = f"/metrics/summary/exchanges/supply"
    response = client.get(url)
    assert response.status_code == 200
    assert response.json() == [
        {
