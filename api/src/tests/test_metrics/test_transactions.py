...
metrics.transactions.TimeWindowLimits = limits
...
def client():
    ...
    sql = f"""
        ...
        -- values = height * 10 + 1
        insert into mtr.transactions (height, daily_1d, daily_7d, daily_28d) values 
...
def base_url(r: str):
    return f"/metrics/transactions?r={r}"
...
def test_summary(client):
    url = f"/metrics/summary/transactions"
    response = client.get(url)
    assert response.status_code == 200
    assert response.json() == [
        {
