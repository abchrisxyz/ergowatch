...
class TestCountAtHeight:
    def test_valid(self, client):
        url = "/utxos/count/at/height/3"
        response = client.get(url)
        assert response.status_code == 200
        assert response.json() == 6

    def test_negative(self, client):
        url = "/utxos/count/at/height/-1"
        response = client.get(url)
        assert response.status_code == 422

    def test_gt_sync_height(self, client):
        url = "/utxos/count/at/height/6"
        response = client.get(url)
...
class TestCountHistory:
    ...
    def test_from_to(self, client):
        url = "/utxos/count/history?from_height=2&to_height=4"
        response = client.get(url)
        assert response.status_code == 200
        assert response.json() == [5, 6, 7]

    def test_from_only(self, client):
        url = "/utxos/count/history?from_height=2"
        response = client.get(url)
        assert response.status_code == 200
        assert response.json() == [5, 6, 7]

    def test_out_of_range(self, client):
        url = "/utxos/count/history?from_height=200"
        response = client.get(url)
        print(response.text)
        assert response.status_code == 200
    ...
    def test_to_only(self, client):
        url = "/utxos/count/history?to_height=4"
        response = client.get(url)
        assert response.status_code == 200
        assert response.json() == [5, 6, 7]

    def test_from_negative(self, client):
        url = "/utxos/count/history?from_height=-5"
        response = client.get(url)
        assert response.status_code == 422

    def test_to_negative(self, client):
        url = "/utxos/count/history?to_height=-5"
        response = client.get(url)
        assert response.status_code == 422
    ...
    def test_from_ge_to(self, client):
        url = "/utxos/count/history?from_height=4&to_height=2"
        response = client.get(url)
        assert response.status_code == 422

    def test_to_gt_sync_height(self, client):
        url = "/utxos/count/history?from_height=4&to_height=6"
        response = client.get(url)
        assert response.status_code == 200
        assert response.json() == [7, 9]

    def test_above_history_limit(self, client):
        url = "/utxos/count/history?from_height=4&to_height=9999999999"
        response = client.get(url)
        assert response.status_code == 422
        response.json()[
            "detail"
