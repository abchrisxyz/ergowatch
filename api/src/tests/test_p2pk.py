...
def test_count_total(client):
    response = client.get("/p2pk/count")
    assert response.status_code == 200
    assert response.json() == 5


def test_count_query_ge(client):
    response = client.get("/p2pk/count?bal_ge=2000000")
    assert response.status_code == 200
    assert response.json() == 4


def test_count_query_lt(client):
    response = client.get("/p2pk/count?bal_lt=4000000")
    assert response.status_code == 200
    assert response.json() == 3


def test_count_query_ge_lt(client):
    response = client.get("/p2pk/count?bal_ge=2000000&bal_lt=4000000")
    assert response.status_code == 200
    assert response.json() == 2


def test_count_zero(client):
    response = client.get("/p2pk/count?bal_ge=999999999999")
    assert response.status_code == 200
...
def test_count_query_lt_cannot_be_negative(client):
    response = client.get("/p2pk/count?bal_lt=-2000000")
    assert response.status_code == 422
